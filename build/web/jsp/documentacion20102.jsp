...
//1018416654
        if (spath.equals("p"))
            ...
            Vector datos = documentos.ConsultaDatos(docu,  confEci.getPeriodo(), oout);
            ...
            if (lon <= 0) {
                ...
                for (int n = 0; n < datos.size(); n++) {
                    ...
                    if (nombres.elementAt(7).equals("N")) {
                ...
                if (estado.equals("-83") && tipo.equals("T")) {
                    ...
                    operacion = "y";
                } else if ((estado.equals("-83") || estado.equals("-60") || estado.equals("-59") ) && (!tipo.equals("T") && blq.equals("1") || blq.equals("2"))) {
                    nombre = "d" + id + ".pdf";
                } else if (acta.equals("No disponible") && (estado.equals("-60") && blq.equals("0"))) {
                    nombre = "ma" + id + ".pdf";
...
<html>
    <head>
        <meta target="principal" http-equiv="Refresh" content="0; url=Documentacion?on=n&spath=spath&docu=<%=docu%>&carnet=<%=nombres.elementAt(0)%>&estado=<%=nombres.elementAt(5)%>">
        <style type="text/css">
        </style>
    ...
    <script language="JavaScript">
        ...
        function validar(formulario, archivo){
            ...
            if(b[b.length-1] == 'pdf'){
                ...
                var x = archivo.split('\\');
                nomar=x[x.length-1];
                document.formulario.action='FileUpload?on=n&spath=correo&fpath=<%=ruta%>&nombre=<%=nombre%>&docu=<%=docu%>&carnet=<%=nombres.elementAt(0)%>&nomestud=<%=nombres.elementAt(2)%>&estado=<%=nombres.elementAt(5)%>&na='+nomar;
                return true;
            }
...
<table align="center" width="740" border="0" cellpadding="0" cellspacing="0" >
    ...
    <table width="85%" border="0" cellpadding="7" cellspacing="0" >
        <tr>
            <td>
                <div align="center">
                    <% if ((estado.equals("-83") || estado.equals("-60")) && (!tipo.equals("T") && loncargue <= 0 && (blq.equals("1") || blq.equals("2")))) {
    ...
    <table width="550" border="0" align="center" cellspacing="0" class="textocom">
        ...
        <td>
    <form name="fmArchivos" method="post" action="Documentacion?on=ar&spath=correo&nombre=<%=nombre%>&docu=<%=docu%>&ref=<%=ref%>">
     ...
     Date fec = new Date(adjuntar.fecArchivo(i));*/
        ...
        <table width="550" border="0" align="center" cellspacing="0" class="textocom">
            <tr>
                ...
                <form enctype="multipart/form-data" method="post" name="formulario" action="FileUpload?on=n&spath=correo&fpath=<%=ruta%>&nombre=<%=nombre%>&docu=<%=docu%>">
